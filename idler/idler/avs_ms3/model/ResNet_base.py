...
class Classifier_Module(nn.Module):
    def __init__(self, dilation_series, padding_series, NoLabels, input_channel):
        ...
        for dilation, padding in zip(dilation_series, padding_series):
            #遍历dilation_series和padding_series中的元素，这两个参数分别代表不同卷积层的扩张系数和填充系数。
            self.conv2d_list.append(nn.Conv2d(input_channel, NoLabels, kernel_size=3, stride=1, padding=padding, dilation=dilation, bias=True))
            #这些卷积层的参数包括输入通道数input_channel、输出通道数NoLabels（类别数目）、卷积核大小为3、步长为1、填充值padding和扩张系数dilation。bias=True表示卷积层使用偏置项。
...
class Pred_endecoder(nn.Module):
    # resnet based encoder decoder继承自PyTorch的nn.Module
    def __init__(self, channel=256, config=None, tpavi_stages=[], tpavi_vv_flag=False, tpavi_va_flag=True):
        #这是类的初始化函数，它接受几个参数，包括预设的通道数channel、一个配置对象config、一个空列表tpavi_stages
        # （可能用于指定TPAVI模块的阶段）、以及两个布尔标志tpavi_vv_flag和tpavi_va_flag，用于控制TPAVI模块的行为。
        super(Pred_endecoder, self).__init__()
        self.cfg = config #将传入的配置对象保存为类的成员变量。
        self.tpavi_stages = tpavi_stages #保存TPAVI模块的阶段列表
        self.tpavi_vv_flag = tpavi_vv_flag #保存TPAVI模块的vv标志。
        self.tpavi_va_flag = tpavi_va_flag #保存TPAVI模块的va标志

        self.resnet = B2_ResNet()#创建一个B2级别的ResNet作为编码器的主体。
        self.relu = nn.ReLU(inplace=True)#创建ReLU激活函数实例，并设置inplace=True以节省内存。
#两个列表分别代表输入和输出特征图中的卷积块数量，输入和输出的卷积块数量是相同的，这意味着每层的结构是对称的
...
##定义输出层，它是一个由卷积层、上采样层和激活函数组成的序列，用于将最终的特征图转换为预测结果
        self.output_conv = nn.Sequential(
            nn.Conv2d(channel, 128, kernel_size=3, stride=1, padding=1),
            Interpolate(scale_factor=2, mode="bilinear"),
            nn.Conv2d(128, 32, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.Conv2d(32, 1, kernel_size=1, stride=1, padding=0),#5, 1, 896, 896
        )
